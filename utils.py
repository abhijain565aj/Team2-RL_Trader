...
def sharpe_ratio(returns, risk_free_rate=0):
    ...
    Parameters:
    returns (np.array): Periodic returns of the asset.
...
def maximum_drawdown(portfolio_values):
    end_index = np.argmax(np.maximum.accumulate(
...
def evaluate_portfolio_performance(agent, logger):
    portfolio_return = agent.portfolio_values[-1] - \
        agent.initial_portfolio_value
    logger.info("--------------------------------")
    logger.info('Portfolio Value:        ${:.2f}'.format(
        agent.portfolio_values[-1]))
    ...
    logger.info(
        'Mean/Daily Return Rate:  {:.3f}%'.format(np.mean(agent.return_rates) * 100))
    ...
    logger.info('Maximum Drawdown:        {:.3f}%'.format(
        maximum_drawdown(agent.portfolio_values) * 100))
    logger.info("--------------------------------")
...
def plot_portfolio_transaction_history(stock_name, agent):
    ...
    plt.title('{} Total Return on {}: ${:.2f}'.format(
        agent.model_type, stock_name, portfolio_return))
    plt.plot(df['Date'], df['Close'], color='black', label=stock_name)
    plt.scatter(agent.buy_dates, buy_prices, c='green', alpha=0.5, label='buy')
    plt.scatter(agent.sell_dates, sell_prices,
                c='red', alpha=0.5, label='sell')
...
def buy_and_hold_benchmark(stock_name, agent):
    ...
    buy_and_hold_return = buy_and_hold_portfolio_values.iloc[-1] - \
        agent.initial_portfolio_value
...
def plot_portfolio_performance_comparison(stock_name, agent):
    ...
    if '^GSPC' not in stock_name:
        dates, GSPC_buy_and_hold_portfolio_values, GSPC_buy_and_hold_return = buy_and_hold_benchmark(
            '^GSPC_2018', agent)
...
def plot_all(stock_name, agent):
    ...
    buy_prices = [df.iloc[t, 4] for t in agent.buy_dates]
    sell_prices = [df.iloc[t, 4] for t in agent.sell_dates]
    ax[0].set_title('{} Total Return on {}: ${:.2f}'.format(
        agent.model_type, stock_name, portfolio_return))
    ax[0].plot(df['Date'], df['Close'], color='black', label=stock_name)
    ax[0].scatter(agent.buy_dates, buy_prices,
                  c='green', alpha=0.5, label='buy')
    ax[0].scatter(agent.sell_dates, sell_prices,
                  c='red', alpha=0.5, label='sell')
...
def plot_portfolio_returns_across_episodes(model_name, returns_across_episodes):
    ...
    plt.grid()
    plt.savefig(
        'visualizations/{}_returns_ep{}.png'.format(model_name, len_episodes))
    plt.show()
